...
function doGet(e) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-xs">
      <div className="bg-slate-900 rounded-2xl border border-slate-800 shadow-2xl max-w-2xl w-full p-6 text-right relative max-h-[90vh] flex flex-col text-slate-100">
        <button
          onClick={onClose}
        ...
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            <h3 className="text-lg font-bold text-white">تحديث كود Google Apps Script لتصدير جميع الأعمدة</h3>
        ...
        <div className="overflow-y-auto flex-1 pr-1 space-y-3 text-xs text-slate-300">
          <div className="p-3 bg-blue-950/60 border border-blue-800 rounded-lg flex items-start gap-2 text-blue-200">
          ...
          <ol className="list-decimal list-inside space-y-1 text-slate-400 mr-2">
            ...
            <li>انسخ الكود البرمجي التالي وضعه مكان دالة <code className="bg-slate-950 px-1 py-0.5 rounded text-amber-300 font-mono border border-slate-800">doGet</code> الحالية:</li>
          </ol>

          <div className="relative">
            <button
              onClick={handleCopy}
              className="absolute top-2.5 left-2.5 z-10 px-2.5 py-1 text-xs font-semibold bg-slate-800 text-white rounded-md hover:bg-slate-700 transition-colors flex items-center gap-1 shadow-sm border border-slate-700 cursor-pointer"
            >
              {copied ? (
                <>
            ...
            <pre className="p-4 bg-slate-950 text-slate-300 rounded-xl overflow-x-auto text-[11px] font-mono leading-relaxed text-left dir-ltr max-h-56 border border-slate-800">
              {scriptCode}
            </pre>
          </div>
          ...
          <p className="text-emerald-300 font-semibold bg-emerald-950/60 p-2.5 rounded-lg border border-emerald-800">
        ...
        <div className="mt-4 pt-3 border-t border-slate-800 flex justify-end">
          <button
            onClick={onClose}
            className="px-4 py-2 text-xs font-bold text-slate-300 bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-lg transition-colors cursor-pointer"
          >
            إغلاق
          </button>
        </div>
