...
export const EditRowModal: React.FC<EditRowModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
      <div className="bg-white rounded-2xl border border-slate-200 shadow-2xl max-w-xl w-full p-6 text-right relative max-h-[90vh] flex flex-col">
        {/* Close button */}
        <button
          onClick={onClose}
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="text-lg font-bold text-slate-900">تعديل بيانات {item.sector}</h3>
        ...
        {/* Form */}
        <form onSubmit={handleSubmit} className="overflow-y-auto flex-1 pr-1 space-y-3.5 text-xs text-slate-700">
          {/* Sector & Status */}
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block font-bold text-slate-700 mb-1">اسم / رقم القطاع *</label>
              <input
                type="text"
                required
                value={sector}
                onChange={(e) => setSector(e.target.value)}
                className="w-full px-3 py-2 border border-slate-300 rounded-lg text-xs font-bold text-slate-900 focus:outline-hidden focus:ring-2 focus:ring-blue-500"
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2 border border-slate-300 rounded-lg text-xs font-semibold text-slate-900 focus:outline-hidden focus:ring-2 focus:ring-blue-500 bg-white"
              >
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-3 py-2 border border-slate-300 rounded-lg text-xs text-slate-900 focus:outline-hidden focus:ring-2 focus:ring-blue-500"
            />
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
              <label className="block font-bold text-slate-700 mb-1">الموقع / المعلم</label>
              <input
                type="text"
                value={location}
          ...
          <div className="pt-3 border-t border-slate-200 flex items-center justify-end gap-2">
            ...
            <button
              type="submit"
              className="px-5 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-bold flex items-center gap-1.5 transition-colors"
            >
